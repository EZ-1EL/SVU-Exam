...
function renderQuestion() {
  ...
  if (item.type === 'mcq' || item.type === 'tf') {
    if (item.type === 'tf') {
      html += `
        <div class="choices">
          <div class="choice" data-i="true">True</div>
          <div class="choice" data-i="false">False</div>
        </div>
      `;
    } else {
      ...
      html += `
        <div class="choices">
          ${shuffledChoices.map(ch => `<div class="choice" data-i="${ch.i}">${ch.c}</div>`).join('')}
        </div>
      `;
    }
    ...
    html += `
      <div class="controls">
        <div class="meta">Question ${index + 1} of ${total}</div>
        <div>
          <button id="skip">Skip</button>
          <button class="secondary" id="showHint">Show Hint</button>
        </div>
      </div>
    ...
    html += `
      <div class="hint" style="font-weight:600;margin-bottom:8px">
        Drag items to order them correctly then press Submit.
      </div>
    `;

    html += `
      <ul id="ordering" class="ordering-list">
        ${item._shuffled.map((it, idx) => `
          <li draggable="true" class="ordering-item" data-pos="${idx}">${it}</li>
        `).join('')}
      </ul>
    `;
  ...
  if (item.type === 'mcq' || item.type === 'tf') {
    document.querySelectorAll('.choice').forEach(el => {
      el.addEventListener('click', e => {
        ...
        if (correct) {
          res.innerHTML = `
            <div style="color:var(--ok);font-weight:700">Correct ✓</div>
          ...
          if (item.type === 'mcq') {
            ...
            res.innerHTML = `
              <div style="color:var(--bad);font-weight:700">
            ...
            res.innerHTML = `
              <div style="color:var(--bad);font-weight:700">Incorrect ✖</div>
              <div class="hint">${item.hint || ''}</div>
            `;
          }
        }
    ...
    document.getElementById('showHint')?.addEventListener('click', () => {
      ...
      if (r) {
        ...
        r.innerHTML = `<div class="hint">${item.hint || ''}</div>`;
      }
    });
  } else if (item.type === 'order') {
    ...
    document.getElementById('submitOrder')?.addEventListener('click', () => {
      ...
      if (ok) {
        res.innerHTML = `
          <div style="color:var(--ok);font-weight:700">Correct order ✓</div>
          <div class="hint">${item.hint || ''}</div>
        `;
        score++;
      } else {
        res.innerHTML = `
          <div style="color:var(--bad);font-weight:700">Incorrect order ✖</div>
          <div style="color:var(--muted);margin-top:6px">Correct order:</div>
          <ol style="color:var(--muted)">
            ${correctSeq.map(s => `<li>${s}</li>`).join('')}
          </ol>
          <div class="hint">${item.hint || ''}</div>
        `;
      }
...
function finishQuiz() {
  ...
  const rv = document.getElementById('reviewBtn');

  if (rv) {
    rv.style.display = 'inline-block';
  }

  const rt = document.getElementById('retryBtn');
  ...
  if (scoreMsg) {
    scoreMsg.textContent = score === total
      ? 'Perfect!'
...
function review() {
  ...
  const html = questions.map((q, i) => {
    ...
    if (q.type === 'mcq') {
      out += `<div style="color:var(--muted)">Answer: ${q.choices[q.ans]}</div>`;
    } else if (q.type === 'tf') {
      out += `<div style="color:var(--muted)">Answer: ${q.ans ? 'True' : 'False'}</div>`;
    } else {
      const correctSeq = q.solution.map(idx => q.items[idx]);
...
document.addEventListener('DOMContentLoaded', () => {
  questions = shuffle(getQuestions().map((q, i) => {
    q._id = i;
    return q;
  }));
